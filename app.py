...
st.set_page_config(page_title="Restaurant Tipping Behavior Analytics", layout="wide")
...
with tab1:
    ...
    day_filter = st.sidebar.multiselect("Day of Week", df['day'].unique(), default=df['day'].unique())
    time_filter = st.sidebar.multiselect("Time of Day", df['time'].unique(), default=df['time'].unique())

    filtered_df = df[
        (df['total_bill'] >= bill_range[0]) & (df['total_bill'] <= bill_range[1]) &
    ...
    with col1:
        ...
        fig_scatter = px.scatter(
            filtered_df, x="total_bill", y="tip", color="smoker", 
            size="size", hover_data=["day", "time"], trendline="ols",
...
with tab2:
    ...
    cat_group = st.selectbox("Select Grouping Variable (Binary)", ["sex", "smoker", "time"], index=1)
    num_target = st.selectbox("Select Target Variable", ["tip", "total_bill"], index=0)
    ...
    if len(groups) == 2:
        ...
        st.write(f"**Normality Check (Shapiro-Wilk):** `{groups[0]}` p={p_s1:.4f}, `{groups[1]}` p={p_s2:.4f}")
        st.write(f"**Homogeneity of Variance Check (Levene's Test):** p={p_lev:.4f}")
        ...
        st.write(f"**Test Statistic:** {stat:.4f} | **p-value:** {p_val:.4e}")
...
with tab3:
    ...
    # Fit OLS model on the complete dataset 'df' to prevent category loss during filtering
    ...
    with col_left:
        ...
        input_bill = st.slider("Total Bill ($)", float(df['total_bill'].min()), float(df['total_bill'].max()), 20.0)
        input_size = st.slider("Party Size", int(df['size'].min()), int(df['size'].max()), 2)
        ...
        st.write(f"**95% Confidence Interval (Mean Tip):** [${pred['mean_ci_lower'].values[0]:,.2f}, ${pred['mean_ci_upper'].values[0]:,.2f}]")
        st.write(f"**95% Prediction Interval (Individual Tip):** [${pred['obs_ci_lower'].values[0]:,.2f}, ${pred['obs_ci_upper'].values[0]:,.2f}]")
    ...
    with col_right:
        ...
        st.write(f"**Residual Normality (Jarque-Bera p-value):** {stats.jarque_bera(model.resid).pvalue:.4e}")
